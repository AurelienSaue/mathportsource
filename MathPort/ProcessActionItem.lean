...
def actionItemToSource (actionItem : ActionItem) : PortM String := do
  ...
  | ActionItem.decl d => do
    ...
    | Declaration.axiomDecl ax => do
      ...
      if s.ignored.contains ax.name then 
        println! "ignored {name}"
        return ""
      
      if s.autoGenerated.contains ax.name then
        println! "autogenerated {name}"
        return ""
      
      println! " PRINTING {name}"
      modify λ s => { s with currLine := s.positionMap.find! ax.name }
      
      return (← liftMetaM $ printAxiomLike "axiom" name ax.levelParams type s.currNamespace ax.isUnsafe)
    ...
    | Declaration.thmDecl thm => do
      ...
      if s.ignored.contains thm.name then 
        println! "ignored {name}"
        return ""
      if s.autoGenerated.contains thm.name then
        println! "autogenerated {name}"
        return ""
      -- maybeRegisterEquation thm.name
      println! " PRINTING {name}"
      modify λ s => { s with currLine := s.positionMap.find! thm.name }
      ...
      if s.sorries.contains thm.name ∨ (¬ (← read).proofs ∧ ¬ s.neverSorries.contains thm.name) then
        ...
        --     isUnsafe := false -- TODO: what to put here?
        -- }
        return (← liftMetaM $ printDefLike "theorem" name thm.levelParams type arbitrary s.currNamespace)
      else
        -- let value ← translate thm.value
        -- addDeclLoud thm.name $ Declaration.thmDecl {
        --   thm with
    ...
    | Declaration.defnDecl defn => do
      ...
      if s.ignored.contains defn.name then 
        println! "ignored {name}"
        return ""
      if ← isBadSUnfold name then 
        println! "bad unfold {name}"
        return ""
      if s.autoGenerated.contains defn.name then
        println! "autogenerated {name}"
        return ""


      println! " PRINTING {name}"
      modify λ s => { s with currLine := s.positionMap.find! defn.name }
      ...
      --     hints := defn.hints
      -- }
      return (← liftMetaM $ printDefLike "def" name defn.levelParams type arbitrary s.currNamespace defn.safety)

    | Declaration.inductDecl lps nps [ind] iu => do
      let name := f ind.name

      if s.ignored.contains ind.name then 
        println! "ignored {name}"
        return ""

      if s.autoGenerated.contains ind.name then
        println! "autogenerated {name}"
        return ""
          
      return (← liftMetaM $ constantToString name s.currNamespace "")
