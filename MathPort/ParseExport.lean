...
def processLine (line : String) : PortM (List ActionItem) := do
  ...
  where
    ...
    processMisc (tokens : List String) : PortM (List ActionItem) := do
      ...
      | ["#POS_INFO", n, line, col]              => do 
        let line ← parseNat line
        let col ← parseNat col
        let name ← str2name n
        
        modify λ s => { s with positionMap := s.positionMap.insert name line}
        if (← get).positions.contains line then
          modify λ s => { s with autoGenerated := s.autoGenerated.insert name }
        else
          modify λ s => { s with positions := s.positions.insert line }
        pure [ActionItem.position name line col]
